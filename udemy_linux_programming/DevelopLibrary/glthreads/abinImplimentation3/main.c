...
int main(int argc, char **argv)
{
	...
	glthread_add_node(emp_list, &emp1->glnode);
	glthread_add_node(emp_list, &emp2->glnode);
	glthread_add_node(emp_list, &emp3->glnode);

	emp_t *ptr = NULL;
	ITERATE_GLTHREAD_BEGIN(emp_list,emp_t,ptr){
		print_emp_details(ptr);
	} ITERATE_GLTHREAD_END;

	glthread_remove_node(emp_list, &emp2->glnode);
	printf("\nprinting again . . . \n");
	...
	ITERATE_GLTHREAD_BEGIN(emp_list, emp_t, ptr){
		...
		glthread_remove_node(emp_list, &ptr->glnode);
		free(ptr);
	} ITERATE_GLTHREAD_END;
	free(emp_list);

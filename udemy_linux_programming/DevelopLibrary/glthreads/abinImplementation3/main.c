...
int main(int argc, char **argv)
{
	...
	add_node_sorted(emp_list, emp1->glnode, emp_t)
	end_add_node_sorted;
	
	add_node_sorted(emp_list, emp2->glnode, emp_t)
        end_add_node_sorted;

	add_node_sorted(emp_list, emp3->glnode, emp_t)
        end_add_node_sorted;
	...
	remove_node(emp_list, &emp2->glnode);
       	remove_node(emp_list, &emp3->glnode);
	free(emp2);free(emp3);	
	printf("\nPrinting again...\n");
